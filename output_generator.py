...
if not(os.path.exists("output/organised_city_data.xlsx")):
    ...
    # Complete the DataFframe for the missing years
    ...
    for year in range(1871,1870,-5):
        ...
        for dep in list(dep_dico.keys()):
            ...
            # Check request success
                ...
                df_temp = pd.read_csv(filename, sep=";", encoding="utf-8", on_bad_lines='skip') # read csv file
                if df_temp.shape[1] != 4:
                    print(f"Badly formed file : {filename}, columns = {df_temp.columns}")
                    continue

                df_temp.columns = ["INSEE", "LIBGEO", "PTOT"+str(year), "Year"]
                df_temp = df_temp[["LIBGEO", "PTOT"+str(year)]] # Filter useful column
                df_temp["DEP"] = dep
                ...
                print(f"Connexion issues for {dep} in {year}: {e}")
